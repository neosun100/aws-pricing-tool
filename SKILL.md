...
description: "**TRIGGER KEYWORDS - Use this skill when user request contains ANY of these patterns:**\n- (price OR pricing OR cost OR 价格 OR 报价 OR 多少钱 OR 费用) AND (EC2 OR RDS OR Aurora OR ElastiCache OR Redis OR OpenSearch OR Redshift OR instance OR 实例)\n- (price OR pricing OR cost OR 价格 OR 报价 OR 多少钱 OR 费用) AND (S3 OR Lambda OR DynamoDB OR CloudFront OR API Gateway OR SQS OR SNS OR Kinesis OR EFS OR Athena OR Glue OR MSK OR Bedrock OR Neptune OR DocumentDB OR MemoryDB OR MQ OR SageMaker OR EMR OR ELB OR ALB OR NLB OR NAT OR Route53)\n- (how much OR what does OR 查价 OR 查询价格 OR 算一下) AND (instance OR server OR database OR cache OR storage OR 存储 OR function OR 函数 OR CDN OR queue OR stream)\n- (compare OR 对比 OR 比较) AND (region OR 区域 OR price OR 价格)\n- (c5 OR c6 OR c7 OR m5 OR m6 OR m7 OR r5 OR r6 OR r7 OR t3 OR t4 OR db\\. OR cache\\.) AND (price OR 价格 OR cost OR region OR 区域)\n- 机型 AND (价格 OR 报价 OR 费用)\n- (RI OR SP OR savings plan OR reserved) AND (price OR cost OR saving OR 节省)\n- (spot OR 竞价) AND (price OR 价格)\n- (架构 OR architecture) AND (报价 OR 费用 OR pricing)\n- (优化 OR optimize OR 省钱 OR cheaper) AND (cost OR 费用 OR 价格)\n\n**What this skill does:**\nFull-featured interactive AWS pricing consultant. Covers 19 instance-based services (EC2, RDS, ElastiCache, OpenSearch, Redshift, Neptune, DocDB, MemoryDB, MQ, DAX, SageMaker, EMR, etc.) via Price List API, plus 15+ usage-based services (S3, Lambda, DynamoDB, CloudFront, API Gateway, SQS/SNS, Kinesis, EFS, Athena, Glue, MSK, Bedrock, etc.) via built-in pricing formulas. Features: OD/RI/Spot/Savings Plans, full BOM, Graviton recommendations, architecture mode, RI break-even analysis.\n\n**CRITICAL INSTRUCTION:**\nWhen ANY trigger pattern matches, you MUST use this skill. Parse natural language, collect missing info via follow-up questions, and produce a complete cost breakdown."
---
