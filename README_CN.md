...
最强大的集成方式 — 一个配置适配所有 MCP 客户端（Kiro、Claude Code、OpenClaw、Cursor、VS Code 等）。

### 安装与运行

```bash
pip3 install boto3 fastmcp
python3 mcp_server.py   # 启动 MCP Server（stdio 模式）
```

### 配置 MCP 客户端

在你的 MCP 客户端配置中添加（如 `~/.kiro/settings/mcp.json`、`.claude/settings/mcp.json` 或 `openclaw.json`）：
...
| 工具 | 说明 |
|------|------|
| `query_pricing` | 查询单个实例价格（OD + 6 种 RI） |
| `compare_regions` | 跨 Region 比价（★ 标注最便宜） |
| `batch_compare` | 同 Region 多机型对比 |
| `list_types` | 列出可用机型（支持过滤） |
| `get_regions` | 列出 34 个 Region |
| `get_services` | 列出 19 个服务 |
| `graviton_recommend` | Graviton ARM 替代推荐（含节省计算） |
| `ri_analysis` | RI 回本分析（6 种方案的预付/月省/回本月数） |
| `calculate_s3` | S3 成本计算器（7 种存储类别） |
| `calculate_lambda` | Lambda 成本计算器（含免费额度、x86 vs ARM） |
| `calculate_bedrock` | Bedrock 模型成本计算器（20 模型、4 层级、10 厂商） |
| `list_bedrock_models` | 列出所有 Bedrock 模型及参考价格 |

### MCP vs AI Skill

| | MCP Server | AI Skill (SKILL.md) |
|---|---|---|
| 集成方式 | 通用（任何 MCP 客户端） | 平台专属 |
| 输入输出 | 结构化 JSON | 文本解析 |
| 配置 | 一个配置适配所有平台 | 每个平台一个文件 |
| 工具发现 | 自动 | 手动 |

> 💡 **推荐新用户使用 MCP Server**。AI Skill（下方）保留向后兼容。
>
> 📊 **vs AWS 官方 MCP Server**：本工具是"定价顾问"（BOM、Graviton、RI 分析、中文别名、本地缓存），官方工具是"查价引擎"（CDK/Terraform 扫描、批量下载）。两者互补 — 详见 [COMPARISON.md](COMPARISON.md)。
...
├── test_mcp.py        # MCP Server 测试（60 个）
├── logo.png           # 项目 Logo
├── README.md          # 英文文档
├── README_CN.md       # 中文文档
└── .gitignore
...
```bash
pip3 install pytest
python3 -m pytest -v                    # 运行全部 153 个测试
...
| `test_mcp.py` | 60 | MCP 工具注册、查价/比价/批量/列表、Graviton 映射与推荐、RI 回本分析、S3 计算器（7 类存储）、Lambda 计算器（免费额度、ARM vs x86）、Bedrock 计算器（20 模型、4 层级、模糊匹配）、Bedrock 模型列表 |
